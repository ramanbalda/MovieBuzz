...
<form action="SaveAdminChat">
        <div class="message-container">
                <%
                DbHandler db=new DbHandler();
        HttpSession httpSession=request.getSession(); 
                httpSession.setAttribute("cid",Integer.parseInt(request.getParameter("id")));
                List<QuestionAns> lst=db.getAllQNAByuserid(Integer.parseInt(request.getParameter("id")));
for(QuestionAns questionAns :lst){
	if(questionAns.getAdmin()==0){
		%>
		<div class="left"><p>User</p><h3><%=questionAns.getMessage() %></h3></div>
		<% 
	}
	else{
		%>
